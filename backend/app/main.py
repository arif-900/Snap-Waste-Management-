...
from fastapi import APIRouter, FastAPI, File, Form, HTTPException, Path, Query, UploadFile, status
...
async def submit_complaint(
    ...
    notes: str | None = Form(None, description="Optional citizen notes")
):
    # Validate file type
    extension = os.path.splitext(image.filename)[1].lower()
    if extension not in [".jpg", ".jpeg", ".png", ".webp"]:
...
def get_all_complaints(
    status: str | None = Query(None, description="Filter by status (Pending, In Progress, Resolved)"),
    waste_type: str | None = Query(None, description="Filter by waste type category")
):
